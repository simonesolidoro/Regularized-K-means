  ...
  void regularize_centroids(std::optional<double> lambda = std::nullopt) {
    for (unsigned c = 0; c < k_; ++c) {
      ...
      if (lambda) {
        ...
        //   optimizer.optimize(model.gcv(edf_cache, 100, seed), lambda_grid_);
        // }
        auto gcv = model.gcv(100, seed);// nuovo gcv non sfrutta cache di edf
...
// RKMeans class for regularized KMeans- con parallel gcv e calcolo distanze
// INSTABILE. errore in cluster per indici fuori da range valido in calcolo distanze parallelo, tanto calcolo distanze è o-piccolo di GCV quindi per ora faccio test con RKMeans_parallel_gcv poi finiti i plot ritorno qui a cercare di aggiustarlo 
template <typename DistancePolicy, typename InitPolicy, typename Triangulation,
          typename Penalty>
class RKMeans_parallel {
private:
  const Eigen::MatrixXd &Y_;
  DistancePolicy dist_;
  InitPolicy init_policy_;
...
public:
  RKMeans_parallel(const DistancePolicy &dist, const InitPolicy &init_policy,
          const Triangulation &triang, const Penalty &penalty,
          const Eigen::MatrixXd &Y, unsigned k = 3,
          unsigned max_iter = MAX_KMEANS_ITERATIONS,
          std::optional<unsigned> seed = std::nullopt)
  ...
  void run(std::optional<double> lambda = std::nullopt) {
    ...
    for (n_iter_ = 0; n_iter_ < max_iter_ && f_changed; ++n_iter_) {
      ...
      singleton_threadpool::instance().parallel_for(start,stop,[&f_changed,this](size_t i, int index_worker){
          ...
          if (best_c != memberships_[i]) {
            memberships_[i] = best_c;
            f_changed.store(true,std::memory_order_relaxed);
          }
        },-1);
...
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
// NON ANCORA IMPLEMNTATO MANCA WRAP DI UPDATE_RESPONSE() DI SOLVER IN MODEL
//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  // RKMeans class for regularized KMeans con parallel gcv e gestione diversa di creazioen SRPDE
  // regularize_centroids() oer ogni c ricrea modello, ma cambia solo risposta non data e penalty--> unico modello per tutti con modifica di risposta con update_response cosi R0,R1 ecc calcolate una volta sola
  //( anche gestione di cache_edf mi sembra strana)
/*template <typename DistancePolicy, typename InitPolicy, typename Triangulation,
          typename Penalty>
class RKMeans_parallel_gcv_2 {
private:
  const Eigen::MatrixXd &Y_;
  DistancePolicy dist_;
  InitPolicy init_policy_;
  ...
  void regularize_centroids(std::optional<double> lambda = std::nullopt) {
    //per il momento solo fuori da for, ma poi da mettere fuori da regularize_centroids()
    GeoFrame data(data_);
    auto &l1 = data.template insert_scalar_layer<POINT>("obs", MESH_NODES);
    l1.load_vec("y", centroids_.row(0));
    SRPDE<typename Penalty::solver_t> model("y ~ f", data, penalty_);
    for (unsigned c = 0; c < k_; ++c) {
      // GeoFrame data(data_);
      // auto &l1 = data.template insert_scalar_layer<POINT>("obs", MESH_NODES);
      ...
      //SRPDE<typename Penalty::solver_t> model("y ~ f", data, penalty_); //perche ricrea modello ogni volta e non fa semplicemente update_response ?
      model.//  MANCA WRAP DI UPDATE_RESPONSE() IN MODEL DI UPDATE_RESPONSE() IN SOLVER !!!!! [LO AGGIUNGERO POI SE CI SARà TEMPO]
      if (lambda) {
        model.fit(0,*lambda);
      } else {
        // calibration
...
public:
  RKMeans_parallel_gcv_2(const DistancePolicy &dist, const InitPolicy &init_policy,
          const Triangulation &triang, const Penalty &penalty,
          const Eigen::MatrixXd &Y, unsigned k = 3,
          unsigned max_iter = MAX_KMEANS_ITERATIONS,
          std::optional<unsigned> seed = std::nullopt)
